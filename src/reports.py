...
def affected_outages_jpg(rows: list[dict[str, Any]], province: str = "Tümü", district: str = "Tümü") -> bytes:
    """Toplu ve sade kesinti çıktısı. Her satır: saha, adres, tarih, süre ve neden."""
    rows = rows or []
    width = 1800
    margin = 50
    ...
    footer_h = 60
    n_rows = (len(rows) + 1) // 2
    height = max(520, header_h + n_rows * (card_h + gap_y) + footer_h + margin)
    img = Image.new("RGB", (width, height), "#f4f6f8")
    draw = ImageDraw.Draw(img)

    draw.rectangle([0, 0, width, header_h], fill="#8b1515")
    draw.text((margin, 28), "YEDAŞ PLANLI KESİNTİ ÇIKTISI", font=_font(30, True), fill="#ffffff")
    subtitle = f"İl: {province}   |   İlçe: {district}   |   Kayıt: {len(rows)}"
    draw.text((margin, 82), subtitle, font=_font(20), fill="#f3e6d8")
    ...
    for idx, row in enumerate(rows):
        ...
        y = header_h + margin // 2 + r * (card_h + gap_y)
        draw.rounded_rectangle([x, y, x + card_w, y + card_h], radius=12, fill="#ffffff", outline="#d7dce2", width=2)

        site = str(row.get("Saha") or "—")
        address = str(row.get("Adres") or "—")
        date = str(row.get("Tarih") or "—")
        duration = str(row.get("Kesinti Süresi") or "—")
        reason = str(row.get("Nedeni") or "—").replace("\n", " ")

        draw.text((x + 18, y + 12), site[:48], font=_font(20, True), fill="#17202a")
        meta = f"{date}  |  {duration}"
        draw.text((x + 18, y + 42), meta[:75], font=label_font, fill="#59636e")
        draw.text((x + 18, y + 64), address[:88], font=value_font, fill="#2f3943")
        wrapped = _wrap(draw, reason, reason_font, card_w - 36)
        draw.text((x + 18, y + 88), wrapped[0][:100], font=reason_font, fill="#6b3030")

    footer_y = height - footer_h + 8
    draw.text((margin, footer_y), f"Oluşturulma: {now_tr().strftime('%d.%m.%Y %H:%M')}", font=_font(14), fill="#68727d")
    return _to_jpg(img)
